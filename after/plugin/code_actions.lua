...
local function select_widget()
	...
	if (label_and_first_parantheses == '(' or label_and_first_parantheses == '') then
	 ...
	 vim.cmd.normal(vim.api.nvim_replace_termcodes('normal ebvf(%', true, true, true))
...
null_ls.register({
            ...
            },
    {
		...
		if (child == nil) then print('Bloc was not able to detect a widget') end
		local input = vim.fn.input("name: ")
		write_to_template(child, [[BlocSelector<{input}Bloc, {input}State, SelectedState>(
                  selector: (state) {
                    return state;
                  },
                  builder: (context, state) {
		...
		if (child == nil) then print('Bloc was not able to detect a widget') end
		local input = vim.fn.input("name: ")
		write_to_template(child, [[BlocListener<{input}Bloc, {input}State>(
                  listener: (context, state) {
		...
		if (child == nil) then print('Bloc was not able to detect a widget') end
		local input = vim.fn.input("name: ")
		write_to_template(child, [[RepositoryProvider(
